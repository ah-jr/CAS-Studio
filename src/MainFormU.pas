...
type
  TMainForm = class(TAcrylicForm)
    ...
    procedure trackWheelUp(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    procedure trackWheelDown(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
...
begin
  ...
  begin
    ...
    begin
      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Width :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset;

      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Controls[0].Left :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset - c_nButtonRight2;
      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Controls[1].Left :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset - c_nButtonRight2;
      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Controls[2].Left :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset - c_nButtonRight1;
      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Controls[3].Left :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset - c_nButtonRight1;

      (m_lstTracks.Items[nIndex] as TAcrylicGhostPanel).Controls[4].Width :=
        sbTracks.ScrollPanel.Width - 2 * c_nPanelOffset - c_nTrackOffset;
    end;
  end;
end;
...
begin
  ...
  if GE_L(Key, $30, $3A) and ((Key - 49) < m_lstTracks.Count) then
    m_CasEngine.GoToTrack
      (StrToInt(String((m_lstTracks.Items[Key - 49] as TAcrylicGhostPanel).Name)
      .SubString(3)));
end;
...
begin
  case TNotificationType(MsgRec.Wparam) of
    ntBuffersDestroyed, ntBuffersCreated, ntDriverClosed:
      ChangeEnabledObjects;
    ntRequestedReset:
      cbDriverChange(cbDriver);
    ntBuffersUpdated:
...
begin
  ...
  btnPlay.Enabled := (m_CasEngine.Ready) and (m_nLoadedTrackCount > 0);
...
begin
  ...
  begin
    try
      m_CasDecoder.AsyncDecodeFile(Handle, odOpenFile.Files,
        m_CasEngine.SampleRate);
  ...
  begin
    ...
    begin
      if (m_CasEngine.ActiveTracks.Count > 0) and
        (m_CasEngine.Database.GetTrackById(m_CasEngine.ActiveTracks.Items[0],
        CasTrack)) then
      begin
        m_CasEngine.Position := CasTrack.Position +
          Trunc(tbProgress.Level * CasTrack.Size);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      tbProgress.Level := m_CasEngine.GetTrackProgress
        (m_CasEngine.ActiveTracks.Items[0]);
      lblTime.Text := m_CasEngine.GetTime + '/' + m_CasEngine.GetDuration;
    end
    else
    begin
  ...
  begin
    if m_CasEngine.Database.GetTrackById
      (StrToInt(String((m_lstTracks.Items[nPanelIdx] as TAcrylicGhostPanel)
      .Name).SubString(3)), CasTrack) then
    begin
      dProgress := (m_CasEngine.Position - CasTrack.Position) / CasTrack.Size;

      ((m_lstTracks.Items[nPanelIdx] as TAcrylicGhostPanel).Controls[4]
        as TAcrylicTrack).Position := dProgress;
      ((m_lstTracks.Items[nPanelIdx] as TAcrylicGhostPanel).Controls[4]
        as TAcrylicTrack).Refresh;
    end;
  end;
end;
...
begin
  ...
  for nPanelIdx := 0 to m_lstTracks.Count - 1 do
  begin
    (m_lstTracks.Items[nPanelIdx] as TAcrylicGhostPanel).Top := nPanelIdx *
      (c_nPanelGap + c_nPanelHeight) + c_nFirstPanelTop;
    ...
    begin
      ...
      ((m_lstTracks.Items[nPanelIdx] as TAcrylicGhostPanel).Controls[4]
        as TAcrylicTrack).Text := IntToStr(nPanelIdx + 1) + ') ' +
        CasTrack.Title;
    end;
  end;
end;
...
begin
  if (a_nTrack1 < m_lstTracks.Count) and (a_nTrack2 < m_lstTracks.Count) and
    (a_nTrack1 <> a_nTrack2) and (a_nTrack1 >= 0) and (a_nTrack2 >= 0) then
  begin
...
begin
  ...
  pnlTrack.Top := m_nLoadedTrackCount * (c_nPanelGap + c_nPanelHeight) +
    c_nFirstPanelTop;
  ...
  AcrylicTrack.Text := IntToStr(m_nLoadedTrackCount + 1) + ') ' +
    a_CasTrack.Title;
...
begin
  if m_CasEngine.Database.GetTrackById
    (StrToInt(String((Sender as TAcrylicButton).Parent.Name).SubString(3)),
    CasTrack) then
  begin
...
var
  nTrack: Integer;
begin
  nTrack := m_lstTracks.IndexOf((Sender as TAcrylicButton)
    .Parent as TAcrylicGhostPanel);
...
begin
  m_CasEngine.GoToTrack(StrToInt(String((Sender as TAcrylicTrack).Parent.Name)
    .SubString(3)));
  UpdateProgressBar;
end;

// ==============================================================================
procedure TMainForm.trackWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  ptMouse: TPoint;
  nTrack: Integer;
...
begin
  ...
  begin
    nTrack := m_lstTracks.IndexOf((Sender as TAcrylicTrack)
      .Parent as TAcrylicGhostPanel);
...
// ==============================================================================
procedure TMainForm.trackWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  ptMouse: TPoint;
  nTrack: Integer;
